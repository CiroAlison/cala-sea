...
const MB_ICONS = {
  ...
  instagram: `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg>`,
  facebook:  `<svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg>`,
  map:       `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>`,
  email:     `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>`,
  none:      ''
};

/* ─── Costruisce la barra mobile da config DB ─── */
function buildMobileBar(buttons) {
  const bar = document.getElementById('mobileBar');
  if (!bar || !Array.isArray(buttons) || !buttons.length) return;
  const enabled = buttons.filter(b => b.enabled !== false);
  if (!enabled.length) return;
  bar.style.gridTemplateColumns = `repeat(${enabled.length}, 1fr)`;
  bar.innerHTML = enabled.map(btn => {
    const isExternal = btn.link && !btn.link.startsWith('#') && !btn.link.startsWith('tel:');
    const isWa = btn.icon === 'whatsapp' || (btn.link || '').includes('wa.me');
    const bgStyle = btn.bg === 'gradient-instagram'
      ? 'background:linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)'
      : `background:${btn.bg || 'rgba(255,255,255,.07)'}`;
    return `<a href="${esc(btn.link || '#')}"
      class="mb-btn"
      style="${bgStyle};color:${btn.fg || '#ffffff'}"
...
(function () {
  ...
  fetch('/api/settings')
    ...
    .then(s => {
      ...
      // === SERVIZI — render dinamico da svc_tiles_json ===
      if (s.svc_tiles_json) {
        try {
          const tiles = JSON.parse(s.svc_tiles_json);
          const car = document.getElementById('serviziCarousel');
      ...
      if (s.mobile_bar_json) {
        try {
          const mbData = JSON.parse(s.mobile_bar_json);
          if (Array.isArray(mbData) && mbData.length) buildMobileBar(mbData);
